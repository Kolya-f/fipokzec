...
const reviews = [
    { text: "fipokzec створив для нас сайт-візитку. Все зроблено якісно, швидко та за адекватною ціною.", author: "Ірина Петренко", position: "Власниця салону краси" },
...
// ========== АНІМАЦІЯ ЕТАПІВ (БРІФ, FIGMA, КОД, ЗАПУСК, ПІДТРИМКА) ==========

// АНІМАЦІЯ БРІФУ
let briefAnimationStarted = false;
document.getElementById('briefContainer').innerHTML = `
    <div style="background:linear-gradient(135deg,#0a0a0f,#0d0d14); border-radius:24px; padding:20px; border:1px solid rgba(127,119,221,0.3); box-shadow:0 20px 40px -15px rgba(0,0,0,0.5);">
        <div style="display:flex; justify-content:space-between; margin-bottom:15px;">
            <div style="display:flex; align-items:center; gap:8px;"><span style="width:8px; height:8px; background:#7F77DD; border-radius:50%; animation:pulseAnim 1.2s infinite;"></span><span style="font-size:10px; color:#7F77DD;">БРИФУВАННЯ</span></div>
            <div style="font-size:10px; color:#555;" id="briefTimerAnim">00:00:00</div>
        </div>
        <div style="height:1px; background:linear-gradient(90deg,transparent,#7F77DD,transparent); margin-bottom:15px;"></div>
        <div style="font-size:17px; margin-bottom:14px; background:linear-gradient(135deg,#fff,#9f97ff); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">📋 Бриф на розробку сайту</div>
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:12px;">
            <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(127,119,221,0.2); border-radius:10px; padding:8px;"><div style="font-size:8px; color:#534AB7;">НАЗВА ПРОЄКТУ</div><div style="font-size:11px; color:#ccc;" id="briefField1"></div></div>
            <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(127,119,221,0.2); border-radius:10px; padding:8px;"><div style="font-size:8px; color:#534AB7;">ТИП САЙТУ</div><div style="font-size:11px; color:#ccc;" id="briefField2"></div></div>
            <div style="grid-column:1/-1; background:rgba(255,255,255,0.03); border:1px solid rgba(127,119,221,0.2); border-radius:10px; padding:8px;"><div style="font-size:8px; color:#534AB7;">ЦІЛЬОВА АУДИТОРІЯ</div><div style="font-size:11px; color:#ccc;" id="briefField3"></div></div>
            <div style="grid-column:1/-1; background:rgba(255,255,255,0.03); border:1px solid rgba(127,119,221,0.2); border-radius:10px; padding:8px;"><div style="font-size:8px; color:#534AB7;">ГОЛОВНА МЕТА</div><div style="font-size:11px; color:#ccc;" id="briefField4"></div></div>
        </div>
        <div style="font-size:8px; color:#444; margin:10px 0 6px;">ПОБАЖАННЯ ДО ДИЗАЙНУ</div>
        <div id="briefChecks" style="margin-bottom:12px;"></div>
        <div style="font-size:8px; color:#444; margin:10px 0 6px;">ПРІОРИТЕТ ФУНКЦІЙ</div>
        <div id="briefBars"></div>
        <div style="margin-top:15px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.05); display:flex; justify-content:space-between; align-items:center;">
            <div style="display:flex; align-items:center; gap:6px;"><div style="width:22px; height:22px; background:rgba(127,119,221,0.2); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:9px;">FP</div><span style="font-size:10px; color:#555;">Менеджер: fipokzec</span></div>
            <div id="briefStatus" style="font-size:9px; padding:2px 10px; border-radius:99px; background:rgba(58,109,17,0.2); border:1px solid rgba(99,153,34,0.3); color:#7fc44a; display:flex; align-items:center; gap:5px; opacity:0;"><span style="width:4px; height:4px; background:#7fc44a; border-radius:50%; animation:pulseAnim 1.2s infinite;"></span>Бриф заповнено</div>
        </div>
        <div style="position:relative; margin-top:12px;"><div style="height:2px; background:rgba(255,255,255,0.05); border-radius:99px;"><div id="briefProgress" style="width:0%; height:100%; background:linear-gradient(90deg,#534AB7,#9f97ff); border-radius:99px;"></div></div></div>
    </div>
`;
...
function startBriefAnimation() {
    ...
    fields.forEach((text, i) => {
        setTimeout(() => {
            ...
            if (el) {
                ...
                let j = 0;
                let iv = setInterval(() => { if (el) el.textContent += chars[j++]; if (j >= chars.length) clearInterval(iv); }, 40);
            }
        }, delay);
        delay += 800;
    });
    ...
    setTimeout(() => {
        let checksHtml = '';
        checks.forEach((c, i) => {
            checksHtml += `<div id="ch${i}" style="display:flex; align-items:center; gap:8px; font-size:11px; color:#666; margin-bottom:5px; opacity:0; transform:translateX(-5px); transition:all 0.35s;"><div style="width:14px; height:14px; border:1px solid rgba(127,119,221,0.3); border-radius:3px;"></div>${c}</div>`;
        });
        ...
        for (let i = 0; i < 4; i++) {
            setTimeout(() => {
                ...
                if (ch) {
                    ...
                    setTimeout(() => {
                        let box = ch.querySelector('div');
                        if (box) { box.style.background = 'rgba(127,119,221,0.2)'; box.style.borderColor = '#7F77DD'; box.innerHTML = '✓'; box.style.display = 'flex'; box.style.alignItems = 'center'; box.style.justifyContent = 'center'; box.style.fontSize = '9px'; box.style.color = '#9f97ff'; ch.style.color = '#888'; }
                    }, 300);
                }
            }, 1700 + i * 350);
        }
    ...
    setTimeout(() => {
        let barsHtml = '';
        bars.forEach((b, i) => {
            barsHtml += `<div style="margin-bottom:8px;"><div style="display:flex; justify-content:space-between; margin-bottom:3px;"><span style="font-size:10px; color:#555;">${b.name}</span><span id="pct${i}" style="font-size:10px; color:#7F77DD;">0%</span></div><div style="height:2px; background:rgba(255,255,255,0.05); border-radius:99px;"><div id="bar${i}" style="width:0%; height:100%; background:linear-gradient(90deg,#534AB7,#9f97ff); border-radius:99px;"></div></div></div>`;
        });
        ...
        bars.forEach((b, i) => {
            setTimeout(() => {
                ...
                let n = 0;
                let iv = setInterval(() => { n = Math.min(n + 3, b.target); if (pct) pct.textContent = n + '%'; if (n >= b.target) clearInterval(iv); }, 30);
            }, 4000 + i * 400);
        });
    }, 3500);
    ...
    let briefInterval = setInterval(() => {
        ...
        let timer = document.getElementById('briefTimerAnim');
        if (timer) timer.textContent = `00:${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}`;
        let prog = document.getElementById('briefProgress');
        if (prog) prog.style.width = Math.min(elapsed / 8000 * 100, 100) + '%';
...
document.getElementById('figmaContainer').innerHTML = `
    <div class="anim-box">
        <div class="anim-header"><div class="dot-pulse" style="display:inline-block; width:8px; height:8px; background:#7F77DD; border-radius:50%; animation:pulseAnim 1.2s infinite;"></div><span style="font-size:10px; color:#7F77DD;">ПРОТОТИПУВАННЯ</span><div class="anim-timer" id="timerFigma" style="margin-left:auto; font-size:10px; color:#555;">00:00:00</div></div>
        <div class="divider" style="height:1px; background:linear-gradient(90deg,transparent,#7F77DD,transparent); margin-bottom:15px;"></div>
        <div class="anim-title" style="font-size:17px; margin-bottom:14px; background:linear-gradient(135deg,#fff,#9f97ff); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">🎨 Прототип та дизайн у Figma</div>
        <div class="figma-canvas" id="figmaCanvas" style="position:relative; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.06); border-radius:12px; height:100px; margin-bottom:10px; overflow:hidden;">
            <div class="figma-cursor" id="figmaCursor" style="position:absolute; left:50%; top:50%; transition:left 0.8s, top 0.8s;"><svg viewBox="0 0 16 16" width="16" height="16"><path d="M2 2l10 4-5 2-2 5-3-11z" fill="#fbbf24" stroke="#0a0a0a" stroke-width="0.5"/></svg></div>
        </div>
        <div class="section-label" style="font-size:8px; color:#444; margin:10px 0 6px;">ШАР: СТРУКТУРА МАКЕТУ</div>
        <div class="figma-layers" id="figmaLayers"></div>
        <div class="section-label" style="font-size:8px; color:#444; margin:10px 0 6px;">УЗГОДЖЕННЯ КОМПОНЕНТІВ</div>
        <div id="figmaChecks"></div>
        <div class="foot" style="margin-top:14px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.05); display:flex; justify-content:space-between; align-items:center;"><div class="ava-row" style="display:flex; align-items:center; gap:6px;"><div class="ava" style="width:22px; height:22px; background:rgba(127,119,221,0.2); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:9px;">FP</div><span class="ava-name" style="font-size:10px; color:#555;">Дизайнер: fipokzec</span></div><div class="status-pill status-blue" id="figmaStatus" style="font-size:9px; padding:2px 10px; border-radius:99px; background:rgba(59,130,246,0.15); border:1px solid rgba(99,152,246,0.3); color:#60a5fa; display:flex; align-items:center; gap:5px; opacity:0;"><span style="width:4px;height:4px;background:#60a5fa;border-radius:50%;animation:pulseAnim 1.2s infinite"></span>Макет узгоджено</div></div>
        <div class="progress-bar" style="height:2px; background:rgba(255,255,255,0.05); border-radius:99px; margin-top:12px;"><div class="progress-fill" id="figmaProgress" style="height:100%; background:linear-gradient(90deg,#534AB7,#9f97ff); border-radius:99px; width:0%; transition:width 0.3s;"></div></div>
    </div>
`;
...
function startFigmaAnimation() {
    ...
    let startTimerFn = (id) => { let start = Date.now(); return setInterval(() => { let e = Date.now()-start; let s=Math.floor(e/1000)%60, m=Math.floor(e/60000)%60; let el=document.getElementById(id); if(el) el.textContent=`00:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`; }, 200); };
    let animProgress = (id, duration) => { let el=document.getElementById(id); let start=Date.now(); let iv=setInterval(()=>{ let pct=Math.min((Date.now()-start)/duration*100,100); if(el) el.style.width=pct+'%'; if(pct>=100) clearInterval(iv); },100); };
    
    startTimerFn('timerFigma');
    animProgress('figmaProgress', 9000);
    
    const canvas = document.getElementById('figmaCanvas');
    const cursor = document.getElementById('figmaCursor');
...
document.getElementById('codeContainer').innerHTML = `
    <div class="anim-box" style="background:linear-gradient(135deg,#0a0a0f,#0d0d14); border-radius:24px; padding:20px; border:1px solid rgba(56,189,248,0.3); box-shadow:0 20px 40px -15px rgba(0,0,0,0.5);">
        <div class="anim-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;"><div class="anim-badge" style="display:flex; align-items:center; gap:8px;"><div class="dot-pulse" style="width:8px; height:8px; background:#38bdf8; border-radius:50%; animation:pulseAnim 1.2s infinite;"></div><span style="font-size:10px; color:#38bdf8;">ВЕРСТКА ТА РОЗРОБКА</span></div><div class="anim-timer" id="timerCode" style="font-size:10px; color:#555;">00:00:00</div></div>
        <div class="divider" style="height:1px; background:linear-gradient(90deg,transparent,#38bdf8,transparent); margin-bottom:15px;"></div>
        <div class="anim-title" style="font-size:17px; margin-bottom:14px; background:linear-gradient(135deg,#fff,#38bdf8); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">💻 Адаптивна верстка та код</div>
        <div class="code-editor" id="codeEditor" style="background:rgba(10,10,20,0.8); border-radius:12px; padding:12px; font-family:'Courier New', monospace; font-size:10px; line-height:1.6; color:#e2e8f0; margin-bottom:10px; min-height:90px;"></div>
        <div class="section-label" style="font-size:8px; color:#444; margin:10px 0 6px;">АДАПТИВНІСТЬ</div>
        <div class="device-row" id="deviceRow" style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:6px; margin-bottom:10px;"><div class="device-card" style="background:rgba(255,255,255,0.03); border:1px solid rgba(127,119,221,0.15); border-radius:8px; padding:8px; text-align:center;"><div class="device-icon" style="font-size:18px;">🖥</div><div class="device-name" style="font-size:9px; color:#555;">Desktop</div><div class="device-status" id="devD" style="font-size:9px; color:#555;">—</div></div><div class="device-card"><div class="device-icon">📱</div><div class="device-name">Mobile</div><div class="device-status" id="devM" style="color:#555">—</div></div><div class="device-card"><div class="device-icon">📟</div><div class="device-name">Tablet</div><div class="device-status" id="devT" style="color:#555">—</div></div></div>
        <div class="section-label" style="font-size:8px; color:#444; margin:10px 0 6px;">ПРОГРЕС ФУНКЦІЙ</div>
        <div id="codeBars"></div>
        <div class="foot" style="margin-top:14px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.05); display:flex; justify-content:space-between; align-items:center;"><div class="ava-row" style="display:flex; align-items:center; gap:6px;"><div class="ava" style="width:22px; height:22px; background:rgba(56,189,248,0.2); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:9px; color:#38bdf8;">FP</div><span class="ava-name" style="font-size:10px; color:#555;">Розробник: fipokzec</span></div><div class="status-pill status-blue" id="codeStatus" style="font-size:9px; padding:2px 10px; border-radius:99px; background:rgba(56,189,248,0.1); border:1px solid rgba(56,189,248,0.3); color:#38bdf8; display:flex; align-items:center; gap:5px; opacity:0;"><span style="width:4px;height:4px;background:#38bdf8;border-radius:50%;animation:pulseAnim 1.2s infinite"></span>Код готовий</div></div>
        <div class="progress-bar" style="height:2px; background:rgba(56,189,248,0.1); border-radius:99px; margin-top:12px;"><div class="progress-fill" id="codeProgress" style="height:100%; background:linear-gradient(90deg,#0284c7,#38bdf8); border-radius:99px; width:0%; transition:width 0.3s;"></div></div>
    </div>
`;
...
function startCodeAnimation() {
    ...
    startTimerFn('timerCode');
    animProgress('codeProgress', 10000);
    ...
    cbars.forEach((b,i)=>{ let div=document.createElement('div'); div.className='bar-row'; div.innerHTML=`<div class="bar-top"><span class="bar-name" style="font-size:10px; color:#555;">${b.name}</span><span class="bar-pct" id="cpct${i}" style="font-size:10px; color:#7F77DD;">0%</span></div><div class="bar-track" style="height:2px; background:rgba(255,255,255,0.05); border-radius:99px;"><div class="bar-fill" id="cbar${i}" style="height:100%; background:linear-gradient(90deg,#534AB7,#9f97ff); border-radius:99px; width:0%; transition:width 1.2s cubic-bezier(0.4,0,0.2,1);"></div></div></div>`;
        cbarsEl.appendChild(div);
...
document.getElementById('launchContainer').innerHTML = `
    <div class="anim-box" style="background:linear-gradient(135deg,#0a0a0f,#0d0d14); border-radius:24px; padding:20px; border:1px solid rgba(251,191,36,0.3); box-shadow:0 20px 40px -15px rgba(0,0,0,0.5);">
        <div class="anim-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;"><div class="anim-badge" style="display:flex; align-items:center; gap:8px;"><div class="dot-pulse" style="width:8px; height:8px; background:#fbbf24; border-radius:50%; animation:pulseAnim 1.2s infinite;"></div><span style="font-size:10px; color:#fbbf24;">ТЕСТУВАННЯ ТА ЗАПУСК</span></div><div class="anim-timer" id="timerLaunch" style="font-size:10px; color:#555;">00:00:00</div></div>
        <div class="divider" style="height:1px; background:linear-gradient(90deg,transparent,#fbbf24,transparent); margin-bottom:15px;"></div>
        <div class="anim-title" style="font-size:17px; margin-bottom:14px; background:linear-gradient(135deg,#fff,#fbbf24); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">🚀 Тестування, SEO та деплой</div>
        <div class="launch-score" id="launchScore" style="display:grid; grid-template-columns:1fr 1fr; gap:6px; margin-bottom:10px;"><div class="score-card" style="background:rgba(255,255,255,0.03); border:1px solid rgba(127,119,221,0.15); border-radius:10px; padding:8px; text-align:center;"><div class="score-num" id="scorePerf" style="font-size:20px; font-weight:700; background:linear-gradient(135deg,#fbbf24,#f59e0b); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">0</div><div class="score-label" style="font-size:9px; color:#555;">Performance</div></div><div class="score-card"><div class="score-num" id="scoreSeo" style="font-size:20px; font-weight:700; background:linear-gradient(135deg,#fbbf24,#f59e0b); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">0</div><div class="score-label">SEO</div></div></div>
        <div class="section-label" style="font-size:8px; color:#444; margin:10px 0 6px;">ЛОГ ДЕПЛОЮ</div>
        <div class="deploy-log" id="deployLog" style="background:rgba(10,10,20,0.8); border-radius:10px; padding:10px; font-family:'Courier New', monospace; font-size:9px; color:#94a3b8; min-height:60px; margin-bottom:10px;"></div>
        <div class="section-label" style="font-size:8px; color:#444; margin:10px 0 6px;">ЧЕКЛИСТ ПЕРЕВІРКИ</div>
        <div class="launch-checklist" id="launchList" style="display:flex; flex-direction:column; gap:5px; margin-bottom:10px;"></div>
        <div class="foot" style="margin-top:14px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.05); display:flex; justify-content:space-between; align-items:center;"><div class="ava-row" style="display:flex; align-items:center; gap:6px;"><div class="ava" style="width:22px; height:22px; background:rgba(251,191,36,0.2); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:9px; color:#fbbf24;">FP</div><span class="ava-name" style="font-size:10px; color:#555;">DevOps: fipokzec</span></div><div class="status-pill" id="launchStatus" style="font-size:9px; padding:2px 10px; border-radius:99px; background:rgba(251,191,36,0.1); border:1px solid rgba(251,191,36,0.3); color:#fbbf24; display:flex; align-items:center; gap:5px; opacity:0;"><span style="width:4px;height:4px;background:#fbbf24;border-radius:50%;animation:pulseAnim 1.2s infinite"></span>Сайт запущено!</div></div>
        <div class="progress-bar" style="height:2px; background:rgba(251,191,36,0.1); border-radius:99px; margin-top:12px;"><div class="progress-fill" id="launchProgress" style="height:100%; background:linear-gradient(90deg,#d97706,#fbbf24); border-radius:99px; width:0%; transition:width 0.3s;"></div></div>
    </div>
`;
...
function startLaunchAnimation() {
    ...
    startTimerFn('timerLaunch');
    animProgress('launchProgress', 9500);
...
document.getElementById('supportContainer').innerHTML = `
    <div class="sw-wrap" style="width:100%">
        <div class="sw-box" style="background:linear-gradient(135deg,#0a0a0f,#0d0d14); border-radius:24px; padding:20px; border:1px solid rgba(29,200,117,0.3); box-shadow:0 20px 40px -15px rgba(0,0,0,0.5);">
            <div class="sw-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;"><div class="sw-badge" style="display:flex; align-items:center; gap:8px;"><div class="dot-g" style="width:8px; height:8px; background:#1dc875; border-radius:50%; animation:pulseG 1.2s infinite;"></div><span style="font-size:10px; color:#1dc875;">ПІДТРИМКА ТА НАВЧАННЯ</span></div><div class="sw-timer" id="swTimer" style="font-size:10px; color:#555;">00:00:00</div></div>
            <div class="divider-g" style="height:1px; background:linear-gradient(90deg,transparent,#1dc875,transparent); margin-bottom:15px;"></div>
            <div class="sw-title" style="font-size:17px; margin-bottom:14px; background:linear-gradient(135deg,#fff,#5eead4); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">🛠️ Адмін-панель та техпідтримка 24/7</div>
            <div class="section-lbl" style="font-size:8px; color:#444; margin:10px 0 6px;">ЧАТ ПІДТРИМКИ — ОНЛАЙН</div>
            <div class="chat-window" id="chatWin" style="background:rgba(10,10,20,0.8); border:1px solid rgba(29,200,117,0.15); border-radius:14px; padding:10px; margin-bottom:10px; min-height:110px;"></div>
            <div class="section-lbl" style="font-size:8px; color:#444; margin:10px 0 6px;">НАВЧАННЯ: АДМІН-ПАНЕЛЬ</div>
            <div class="admin-wrapper"><div class="admin-screen" id="adminScreen" style="background:rgba(15,15,25,0.9); border:1px solid rgba(255,255,255,0.07); border-radius:12px; overflow:hidden; margin-bottom:10px;"><div class="admin-bar" style="background:rgba(30,30,45,0.9); padding:7px 12px; display:flex; align-items:center; gap:8px;"><div class="admin-bar-dot" style="width:7px; height:7px; border-radius:50%; background:#ff5f56;"></div><div class="admin-bar-dot" style="width:7px; height:7px; border-radius:50%; background:#ffbd2e;"></div><div class="admin-bar-dot" style="width:7px; height:7px; border-radius:50%; background:#27c93f;"></div><div class="admin-bar-url" style="font-size:9px; color:#555; margin-left:6px; font-family:monospace;">site.com/wp-admin/</div></div><div class="admin-body" style="display:flex; height:90px;"><div class="admin-sidebar" style="width:80px; background:rgba(20,20,35,0.9); border-right:1px solid rgba(255,255,255,0.05); padding:8px 0;"><div class="admin-menu-item" id="mi0" style="font-size:9px; color:#555; padding:5px 10px; display:flex; align-items:center; gap:5px;">📄 Сторінки</div><div class="admin-menu-item" id="mi1">📝 Записи</div><div class="admin-menu-item" id="mi2">🖼 Медіа</div><div class="admin-menu-item" id="mi3">⚙️ Налаш.</div></div><div class="admin-content" id="adminContent" style="flex:1; padding:10px;"><div class="admin-row" style="height:10px; background:rgba(255,255,255,0.04); border-radius:4px; margin-bottom:6px; width:80%"></div><div class="admin-row" style="width:60%"></div><div class="admin-row admin-highlight" id="adminHL" style="width:90%;height:14px;background:rgba(29,200,117,0.15);border:1px solid rgba(29,200,117,0.3);border-radius:6px;opacity:0"></div><div class="admin-row" style="width:45%"></div></div></div></div></div>
            <div class="section-lbl" style="font-size:8px; color:#444; margin:10px 0 6px;">АПТАЙМ ОСТАННІ 30 ДНІВ</div>
            <div class="uptime-dots" id="uptimeDots" style="display:flex; gap:3px; margin-bottom:10px; flex-wrap:wrap;"></div>
            <div class="resp-grid" style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:6px; margin-bottom:10px;"><div class="resp-card" style="background:rgba(255,255,255,0.03); border:1px solid rgba(29,200,117,0.15); border-radius:10px; padding:8px; text-align:center;"><div class="resp-num" id="rTime" style="font-size:18px; font-weight:700; background:linear-gradient(135deg,#1dc875,#5eead4); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">—</div><div class="resp-label" style="font-size:9px; color:#555;">Час відповіді</div></div><div class="resp-card"><div class="resp-num" id="rUptime">—</div><div class="resp-label">Аптайм</div></div><div class="resp-card"><div class="resp-num" id="rTickets">—</div><div class="resp-label">Закрито тікетів</div></div></div>
            <div class="section-lbl" style="font-size:8px; color:#444; margin:10px 0 6px;">НАВЧАННЯ КЛІЄНТА — ПРОГРЕС</div>
            <div id="swBars"></div>
            <div class="foot" style="margin-top:14px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.05); display:flex; justify-content:space-between; align-items:center;"><div class="ava-row" style="display:flex; align-items:center; gap:6px;"><div class="ava" style="width:22px; height:22px; background:rgba(29,200,117,0.2); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:9px; color:#1dc875;">FP</div><span class="ava-name" style="font-size:10px; color:#555;">Підтримка: fipokzec 24/7</span></div><div class="status-pill" id="swStatus" style="font-size:9px; padding:2px 10px; border-radius:99px; background:rgba(29,200,117,0.1); border:1px solid rgba(29,200,117,0.3); color:#1dc875; display:flex; align-items:center; gap:5px; opacity:0;"><span style="width:4px;height:4px;background:#1dc875;border-radius:50%;animation:pulseG 1.2s infinite"></span>Клієнт навчений ✓</div></div>
            <div class="progress-bar" style="height:2px; background:rgba(29,200,117,0.1); border-radius:99px; margin-top:12px;"><div class="progress-fill" id="swProgress" style="height:100%; background:linear-gradient(90deg,#0d6b46,#1dc875); border-radius:99px; width:0%; transition:width 0.3s;"></div></div>
        </div>
    </div>
`;
...
function startSupportAnimation() {
    ...
    setInterval(() => {
        const e = Date.now()-startSw;
        const s=Math.floor(e/1000)%60, m=Math.floor(e/60000)%60;
        const el=document.getElementById('swTimer');
        if(el) el.textContent=`00:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;
    }, 200);
    
    const progElSw = document.getElementById('swProgress');
    const pStartSw = Date.now();
    setInterval(() => {
        const pct = Math.min((Date.now()-pStartSw)/10000*100,100);
        if(progElSw) progElSw.style.width=pct+'%';
    }, 100);
    
    const msgs = [
        { from:'client', text:'Привіт! Як додати новий пост?', delay:400 },
        { from:'dev',    text:'Зараз покажу! Заходьте в Записи → Додати новий', delay:1800 },
        { from:'client', text:'Знайшов, але де змінити зображення?', delay:3200 },
        { from:'dev',    text:'Медіа → Завантажити — просто перетягніть файл 🖼', delay:4600 },
        { from:'client', text:'Супер! Дякую, все зрозуміло 🙌', delay:6000 },
    ];
    ...
    msgs.forEach(m => {
        setTimeout(() => {
            ...
            div.innerHTML = `<div class="msg-ava ${isClient ? 'ava-client' : 'ava-dev'}">${isClient ? 'КЛ' : 'FP'}</div><div class="msg-bubble ${isClient ? 'bubble-client' : 'bubble-dev'}">${m.text}</div>`;
        ...
        if (m.from === 'dev') {
            setTimeout(() => {
                ...
                typing.id = `typ${m.delay}`;
                typing.innerHTML = `<div class="msg-ava ava-dev">FP</div><div class="typing-bubble"><div class="typing-dot"></div><div class="typing-dot"></div><div class="typing-dot"></div></div>`;
                chatWin.appendChild(typing);
                chatWin.scrollTop = chatWin.scrollHeight;
                setTimeout(() => { const t = document.getElementById(`typ${m.delay}`); if(t) t.remove(); }, 1300);
            }, m.delay - 1350);
        }
    });
    
    const menuItems = ['mi0','mi1','mi2','mi3'];
    let curMenu = 0;
    setInterval(() => {
        menuItems.forEach((id,i) => {
            const el = document.getElementById(id);
            if(el) el.classList.toggle('active', i===curMenu);
        });
        curMenu = (curMenu+1)%4;
        const hl = document.getElementById('adminHL');
        if(hl) {
            hl.style.opacity='1';
            setTimeout(() => { if(hl) hl.style.opacity='0'; }, 800);
        }
    }, 1800);
    ...
    for(let i=0;i<30;i++) {
        ...
        setTimeout(() => {
            ...
            d.classList.add(roll > 0.07 ? 'ok' : 'warn');
        }, 500 + i*80);
    }
    ...
    const barsSw = [
        { name:'Публікація матеріалів', target:100 },
        { name:'Управління медіафайлами', target:95 },
        { name:'Налаштування плагінів', target:80 },
        { name:'Базове SEO у CMS', target:70 },
    ];
    const barsElSw = document.getElementById('swBars');
    barsSw.forEach((b,i) => {
        const div = document.createElement('div');
        div.className = 'bar-row';
        div.innerHTML = `<div class="bar-top"><span class="bar-name" style="font-size:10px; color:#555;">${b.name}</span><span class="bar-pct" id="spct${i}" style="font-size:10px; color:#1dc875;">0%</span></div><div class="bar-track" style="height:2px; background:rgba(255,255,255,0.05); border-radius:99px;"><div class="bar-fill" id="sbar${i}" style="height:100%; background:linear-gradient(90deg,#0d6b46,#1dc875); border-radius:99px; width:0%; transition:width 1.2s cubic-bezier(0.4,0,0.2,1);"></div></div></div>`;
        barsElSw.appendChild(div);
        setTimeout(() => {
            const bar=document.getElementById(`sbar${i}`);
            if(bar) bar.style.width=b.target+'%';
            let n=0;
            const iv=setInterval(() => {
                n=Math.min(n+2,b.target);
                const p=document.getElementById(`spct${i}`);
                if(p) p.textContent=n+'%';
                if(n>=b.target) clearInterval(iv);
            }, 25);
        }, 5000+i*400);
    });
...
// СПОСТЕРЕЖЕННЯ ЗА СКРОЛОМ ДЛЯ ЕТАПІВ
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Запуск анімації навичок
    setTimeout(() => {
        setupSkillsAnimation();
    }, 500);
    
    // Запуск анімації етапів при скролі
