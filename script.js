// Бібліотека різних кодів (кольорові, з синтаксисом)
const codeSamples = [
    {
...
<span style="color:#fbbf24">&lt;html&gt;</span>
  <span style="color:#fbbf24">&lt;head&gt;</span>
    ...
    <span style="color:#fbbf24">&lt;style&gt;</span>
      ...
      <span style="color:#60a5fa">.btn:hover</span> { <span style="color:#c084fc">transform</span>: <span style="color:#60a5fa">scale(1.05)</span>; }
    <span style="color:#fbbf24">&lt;/style&gt;</span>
  <span style="color:#fbbf24">&lt;/head&gt;</span>
...
<span style="color:#fbbf24">const</span> <span style="color:#60a5fa">App</span> = () => {
  ...
  <span style="color:#fbbf24">return</span> (
    <span style="color:#fbbf24">&lt;div&gt;</span>
      ...
      <span style="color:#fbbf24">&lt;button onClick={() => setCount(count + 1)}&gt;</span>
        +1
      <span style="color:#fbbf24">&lt;/button&gt;</span>
    <span style="color:#fbbf24">&lt;/div&gt;</span>
  );
};
...
app.<span style="color:#60a5fa">listen</span>(<span style="color:#c084fc">3000</span>, () => {
  ...
  <span style="color:#60a5fa">.glass-card</span> {
    ...
    <span style="color:#c084fc">border</span>: <span style="color:#60a5fa">1px solid rgba(255,255,255,0.2)</span>;
...
// Оновлені послуги (без смайликів, з новими цінами та послугами)
const services = [
    { title: "Сайт-візитка", description: "3-5 сторінок, форма зв'язку, адаптивний дизайн.", features: ["Адаптивний дизайн", "Форма зв'язку", "SEO-оптимізація", "Контактна інформація"], price: "5 000 грн" },
    { title: "Лендінг", description: "Односторінковий сайт для запуску продукту або акції.", features: ["Анімації", "Форма захоплення", "Інтеграція з CRM", "Аналітика"], price: "6 000 грн" },
    ...
    { title: "Підтримка", description: "Щомісячне оновлення, бекапи, технічний супровід.", features: ["Щомісячні оновлення", "Резервні копії", "Техпідтримка 24/7", "Безпека"], price: "1 000 грн/міс" },
    ...
    { title: "Оновлення дизайну", description: "Рестайлінг, сучасний вигляд, покращення UX/UI.", features: ["Новий дизайн", "Адаптація", "Покращення навігації", "Сучасні анімації"], price: "2 000 грн" }
];

const portfolioItems = [
    { title: "Стоматологія SmileCare", imageBg: "linear-gradient(135deg, #1e3a8a, #3b82f6)", tech: ["WordPress", "Elementor"], link: "#", category: "wordpress" },
...
const reviews = [
    { text: "Коля створив для нас сайт-візитку. Все зроблено якісно, швидко та за адекватною ціною. Рекомендую!", author: "Ірина Петренко", position: "Власниця салону краси" },
    { text: "Дуже задоволений роботою! Інтернет-магазин працює ідеально, клієнти залишають позитивні відгуки.", author: "Олександр Коваль", position: "Підприємець" },
    { text: "Професійний підхід, увага до деталей та відмінна підтримка після запуску. Дякую!", author: "Марія Шевченко", position: "Маркетолог" }
];

const faqItems = [
    { question: "Скільки часу займає створення сайту?", answer: "Сайт-візитка — 5-7 днів, лендінг — 7-10 днів, інтернет-магазин — 14-21 день." },
    { question: "Чи можна буде самостійно редагувати сайт?", answer: "Так, я навчаю клієнтів працювати з адмін-панеллю. Ви зможете самостійно змінювати тексти, фото та інше." },
    { question: "Яка гарантія на роботу?", answer: "Надаю гарантію 3 місяці на всі види робіт. Безкоштовно виправляю помилки, якщо вони виникли з моєї вини." },
        ...
        <div class="portfolio-item">
            ...
            <div class="portfolio-content">
                ...
                <div style="display:flex;gap:8px;margin:12px 0">
                    ${i.tech.map(t => `<span style="background:rgba(251,191,36,0.2);padding:4px 12px;border-radius:40px;font-size:0.7rem">${t}</span>`).join('')}
                </div>
                <a href="${i.link}" style="color:#fbbf24;text-decoration:none">Переглянути →</a>
            </div>
        </div>
    `).join('');
}
...
function renderReviews() {
    ...
    if (container) container.innerHTML = reviews.map(r => `
        <div class="review-card">
            <div class="review-text">“${r.text}”</div>
            <div class="review-author" style="color:#fbbf24;margin-top:12px;font-weight:600">${r.author}</div>
            <div class="review-position" style="color:#94a3b8;font-size:0.8rem">${r.position}</div>
        </div>
    `).join('');
}
...
function renderFAQ() {
    ...
    if (container) container.innerHTML = faqItems.map(f => `
        <div class="faq-item">
            <div class="faq-question" style="color:#fbbf24;font-weight:700;margin-bottom:8px">❓ ${f.question}</div>
            <div class="faq-answer" style="color:#94a3b8">${f.answer}</div>
        </div>
    `).join('');
}
